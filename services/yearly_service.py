"""Yearly dashboard business logic and persistence helpers."""
...
def get_yearly_data():
    ...
    year_meta = {int(row["calendar_year"]): row for row in annual_rows}
...
def get_yearly_commentary(calendar_year):
    with db_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
...
def calculate_year(calendar_year):
    current_year = datetime.now(timezone.utc).year

    if calendar_year == 2012:
        raise ValueError("2012 is reserved for historical workbook import and cannot be calculated.")
    if calendar_year < 2013:
        raise ValueError("calendar_year must be >= 2013.")
...
def validate_year_calculation_input(calendar_year):
    current_year = datetime.now(timezone.utc).year
